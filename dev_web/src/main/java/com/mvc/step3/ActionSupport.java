...
/*
 *  리턴 타입을 String + ModelAndView 추가 지원해 본다.
 *  CRUD 구현에 필요한 로직을 if문이 아닌 메소드 중심 코딩 전개가 가능하도록 개선해 본다
 *  : 메소드 안에  req와 res를 지원해야 함 - HttpSession session = req.getSession
...
public class ActionSupport extends HttpServlet {
	...
	private void doService(HttpServletRequest req, HttpServletResponse res)
		...
		// 1-2,3 과는 다르게 메소드의 파라미터로  upmu 배열을 전달함
//		req.setAttribute("upmu", upmu); // 따라서 여기는 필요X 왜냐면 파라미터로 전달하니까
		//업무에 대응하는 컨트롤러 클래스에 대한 인ㅌ스턴스화는 HandlerMapping 클래스에서 하니까
		// 아래 코드도 필요X
