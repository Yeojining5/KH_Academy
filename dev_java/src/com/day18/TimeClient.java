...
public class TimeClient extends Thread {
	...
	public void run() {
		...
		try {
			...
			socket = new Socket("192.168.0.4", 3000);
			//생성된 소켓이 있어야 출력을 담당하는 객체를 생성할 수 있다.
			//생성자 파라미터 자리에 소켓.getOutputStream()으로 객체생성됨 - 말하기 - 쓰기(현재시간정보를 소켓에 쓴다)
			oos = new ObjectOutputStream(socket.getOutputStream());
			// 서버측에서 보낸 메세지를 읽어오는데 사용한다 - 객체를 생성할 때 반드시 Socket객체가 생성되어 있어야 함.
