...
// 인터페이스는 생성자를 가질 수 없지만 추상클래스는 생성자를 가질 수 있다. (convention)
// 일반 메소드, 추상메소드 모두를 가짐
// 따라서 추상메소드 앞에 abstract 는 절대로 생략할 수 없다. (일반메소드와 구분해야 하므로)
public abstract class Car implements MoveBehavior {

	public int speed = 0;
	public int wheelNum = 0;
	// 어떤 자동차가 될지를 정할 수 없어서 추상메소드로 하기로 함 (아직 미결정)
	public abstract void display(); // 추상메소드
	
	// 좌중괄호와 우중괄호가 있는 메소드는 일반 메소드임
	@Override
	public void run() { // 엑셀을 밟으면 그 때마다 호출된다.
		speed = speed +1;
	}
}
 ...
 * 		왜냐하면 추상메소드만 가지니까
 * 인터페이스와 추상클래스의 공통점은 둘 다 단독으로 인스턴스화 불가함
 * 		이것 때문에 다형성이 가능한 것이다.
 ...
 * 단 자바는 이럴 경우 생성부의 타입으로 객체가 생성된다. 메모리에 로딩 된다.
 * 
 * 		클래스 타입 변수명 = new 클래스 타입(); - 스탠다드
 * 		추상클래스 변수명 = new 구현체클래스(); - 잘하는사람 > 다형성이 가능해짐 > 코드 수정이 줄어듦
