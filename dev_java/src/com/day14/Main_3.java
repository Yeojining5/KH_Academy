...
// 메인에서 입력,수정,상세보기 라는 3가지 업무에 대해서 Dialog_3 하나의 클래스를 
// 단 한번만 인스턴스화 한 자료를 가지고 모두 처리를 하고 싶다. 어떡하지?
// 입력할때는 빈칸을 제공하고 새로입력한 값을 저장해야 한다. - 업무 역할
...
public class Main_3 extends JFrame implements ActionListener{
   // 메인화면에 사용할 컴포넌트들을 선언합니다.
    ...
    public Main_3() {
       initDisplay(); // 화면을 배치하고 출력해주는 메소드
    }
   public void initDisplay() {
      dialog = new Dialog_3(this);  // this 1_2에 없었음
       this.setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
       this.setLayout(new FlowLayout());
       btnInsert.addActionListener(this); // 사용자가 btn=버튼을 누르면 JVM이 감지함
       btnUpdate.addActionListener(this);
       btnDetail.addActionListener(this);
// 버튼을 누르면 JVM이 감지하고 Actionperformed 수행된다 
       ...
       this.setVisible(true); // 화면에 보여주는 메소드 / 보여주기 싫으면 false
   }
   public void refreshData() { // F5 새로고침과 비슷한 메소드
      System.out.println("refreshData 호출 성공");
      // 새로고침 후에 자손 화면을 닫기(자손클래스의 자원 반납을 받는 메소드 호출)
      dialog.dispose(); // 얘는 어디로 갔을까? 생성자로 이전함!!
   }
